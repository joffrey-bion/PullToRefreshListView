...
class SwipeListViewTouchListener implements View.OnTouchListener {
	...
	protected void swipe(int position) {
		...
		if (position >= first && position <= last) {
			...
			openAnimate(listView.getChildAt(position - first).findViewById(opts.frontViewId), position);
		} else {
	...
	protected void unswipe(int position) {
		...
		if (position >= first && position <= last) {
			// the affected view is visible
			closeAnimate(listView.getChildAt(position - first).findViewById(opts.frontViewId), position);
		} else {
	...
	protected void performDismiss(final View dismissView, final int dismissPosition, boolean doPendingDismiss) {
		...
		ValueAnimator animator = ValueAnimator.ofInt(originalHeight, 1).setDuration(opts.animationTime);
	...
	private void animateMovingItem(final boolean changeState, final boolean toRight) {
		Log.d(LOG_TAG, "Animation: " + (changeState ? "swiping " + (toRight ? "right" : "left") : "releasing")
				+ " item " + movingItem.position);
		int action = swiped.get(movingItem.position) ? SwipeOptions.ACTION_REVEAL : toRight ? currentActionRight
				: currentActionLeft;
	 ...
	 *            list position
	 */
	private void animateReveal(final View view, final boolean changeState, final boolean toRight, final int position) {
	 ...
	 *            Position of list
	 */
	private void animateDismiss(final View view, final boolean swap, final boolean swapRight, final int position) {
		int moveTo = 0;
		if (swiped.get(position)) {
			if (!swap) {
	...
	private int getSwipedOffset(boolean swapRight) {
		if (opts.swipeOffsetType == SwipeOptions.OFFSET_TYPE_TRAVELED) {
			...
			return swapRight ? (int) (viewWidth - opts.swipeOffsetRight) : (int) (-viewWidth + opts.swipeOffsetLeft);
		}
	}
	...
	private void updateCurrentAction() {
		...
		movingItem.backView.setVisibility(currentAction == SwipeOptions.ACTION_CHOICE ? View.GONE : View.VISIBLE);
	}
	...
	public boolean shouldIntercept(MotionEvent ev) {
		...
		if (isSwipeEnabled()) {
			...
			case MotionEvent.ACTION_MOVE:
				...
				Log.d(LOG_TAG, "Intercept MOVE " + (currentMotion.scrollState == STATE_SCROLLING_X) + " (state="
	...
	public boolean onTouch(View view, MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP: {
			...
			if (validSwipe) {
				toRight = deltaX > 0;
				Log.i(LOG_TAG, "Swipe item " + movingItem.position + " to " + (toRight ? "right" : "left") + "!");
			} else if (validFling) {
				...
				Log.i(LOG_TAG, "Fling item " + movingItem.position + " to " + (toRight ? "right" : "left") + "!");
			} else {
	...
	private boolean isAllowedDirection(boolean toRight, int position) {
		...
		if (swiped.get(position)) {
			...
			if ((!swipedRight && !toRight) || (swipedRight && toRight)) {
				...
				Log.v(LOG_TAG, "Drag blocked: trying to unswipe a " + (swipedRight ? "right" : "left")
						+ "-swiped element to the " + (toRight ? "right" : "left"));
				return false;
			}
			...
			Log.v(LOG_TAG, "Drag blocked: trying to swipe the element to the " + (toRight ? "right" : "left"));
			return false;
		}
		return true;
	}
	...
	private void moveMovingItemToPosition(float targetX) {
		...
		if (currentAction == SwipeOptions.ACTION_DISMISS) {
			...
			setAlpha(movingItem.view, Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(targetX) / viewWidth)));
	...
	private class Motion {
		...
		public boolean isValidXFling() {
			...
			if (velocityX < minFlingVelocity || velocityX > maxFlingVelocity || velocityY * 2 > velocityX) {
	...
	private static void animate(View v, float translationX, long animationTime, final Runnable animationEndCallback) {
		v.animate().translationX(translationX).setDuration(animationTime).setListener(new AnimatorListenerAdapter() {
			@Override
			public void onAnimationEnd(Animator animation) {
