 ...
 * animated values and set them on target objects.
 * 
 * <p>
...
public class ValueAnimator extends Animator {
     ...
     * FRAME is sent by the handler to itself to process the next animation frame
     */
     ...
     * deep copy of any referenced objects. As other non-trivial fields are added to
    ...
    // every update.
    // If ThreadLocal access is significantly expensive, we may want to put all of
    // these
    // fields into a structure sot hat we just access ThreadLocal once to get the
    // reference
    // to that structure, then access the structure directly for each field.

    // The static sAnimationHandler processes the internal timing loop on which all
    // animations
    // are based
    private static ThreadLocal<AnimationHandler> sAnimationHandler = new ThreadLocal<AnimationHandler>();

    // The per-thread list of all active animations
    static final ThreadLocal<ArrayList<ValueAnimator>> sAnimations = new ThreadLocal<ArrayList<ValueAnimator>>() {
        @Override
    ...
    static final ThreadLocal<ArrayList<ValueAnimator>> sPendingAnimations = new ThreadLocal<ArrayList<ValueAnimator>>() {
        @Override
        protected ArrayList<ValueAnimator> initialValue() {
            return new ArrayList<ValueAnimator>();
        }
     ...
     */
    static final ThreadLocal<ArrayList<ValueAnimator>> sDelayedAnims = new ThreadLocal<ArrayList<ValueAnimator>>() {
        @Override
        protected ArrayList<ValueAnimator> initialValue() {
            return new ArrayList<ValueAnimator>();
        }
    ...
    static final ThreadLocal<ArrayList<ValueAnimator>> sEndingAnims = new ThreadLocal<ArrayList<ValueAnimator>>() {
        @Override
        protected ArrayList<ValueAnimator> initialValue() {
            return new ArrayList<ValueAnimator>();
        }
    ...
    static final ThreadLocal<ArrayList<ValueAnimator>> sReadyAnims = new ThreadLocal<ArrayList<ValueAnimator>>() {
        @Override
        protected ArrayList<ValueAnimator> initialValue() {
            return new ArrayList<ValueAnimator>();
        }
    ...
    /**
     * Used to indicate whether the animation is currently playing in reverse. This
     * causes the elapsed fraction to be inverted to calculate the appropriate
     * values.
     */
    ...
    /**
     * Tracks the time at which the animation began playing through its startDelay.
     * This is different from the mStartTime variable, which is used to track when
     * the animation became active (which is when the startDelay expired and the
     * animation was added to the active animations list).
     */
    private long mDelayStartTime;

    /**
     * Flag that represents the current state of the animation. Used to figure out
     * when to start an animation (if state == STOPPED). Also used to end an
     ...
     * There is some lag between a call to start() and the first animation frame. We
     * should still note that the animation has been started, even if it's first
     * animation frame has not yet happened, and reflect that state in isRunning().
     * Note that delayed animations are different: they are not started until their
     * first animation frame, which occurs after their delay elapses.
     */
    ...
    static long sFrameDelay = DEFAULT_FRAME_DELAY;

    // The number of times the animation will repeat. The default is 0, which means
    // the animation
    // will play only once
    private int mRepeatCount = 0;

    /**
     * The type of repetition that will occur when repeatMode is nonzero. RESTART
     * means the animation will start from the beginning on every new cycle. REVERSE
     * means the animation will reverse directions on each iteration.
     */
    private int mRepeatMode = RESTART;

    /**
     * The time interpolator to be used. The elapsed fraction of the animation will
     * be passed through this interpolator to calculate the interpolated fraction,
     * which is then used to calculate the animated values.
     */
     ...
     * property being animated). Therefore, there should typically be two or more
     * values.
     * 
     * @param values
     *            A set of values that the animation will animate between over time.
     * @return A ValueAnimator object that is set up to animate between the given
     *         values.
     */
    ...
    /**
     * Sets int values that will be animated between. A single value implies that
     ...
     * 
     * <p>
     * If there are already multiple sets of values defined for this ValueAnimator
     * via more than one PropertyValuesHolder object, this method will set the values
     ...
     * internally by the constructors of ValueAnimator that take a list of values.
     * But an ValueAnimator can be constructed without values and this method can be
     * called to set the values manually instead.
     * 
     * @param values
     ...
     * 
     * @param duration
     *            The length of the animation, in milliseconds. This value cannot be
     *            negative.
     * @return ValueAnimator The object called with setDuration(). This return value
     *         makes it easier to compose statements together that construct and then
     *         set the duration, as in
     ...
     * 
     * @param playTime
     *            The time, in milliseconds, to which the animation is advanced or
     *            rewound.
     */
    ...
    /**
     * The most recent value calculated by this <code>ValueAnimator</code> when there
     * is just one property being animated. This value is only sensible while the
     * animation is running. The main purpose for this read-only property is to
     * retrieve the value from the <code>ValueAnimator</code> during a call to
     * {@link AnimatorUpdateListener#onAnimationUpdate(ValueAnimator)}, which is
     * called during each animation frame, immediately after the value is calculated.
     * 
     * @return animatedValue The value most recently calculated by this
     *         <code>ValueAnimator</code> for the single property being animated. If
     *         there are several properties being animated (specified by several
     *         PropertyValuesHolder objects in the constructor), this function
    ...
    public Object getAnimatedValue() {
        ...
        // Shouldn't get here; should always have values unless ValueAnimator was set
        // up wrong
        return null;
    }

    /**
     * Sets how many times the animation should be repeated. If the repeat count is
     * 0, the animation is never repeated. If the repeat count is greater than 0 or
     * {@link #INFINITE}, the repeat swipeMode will be taken into account. The repeat
    ...
    @Override
    public void cancel() {
        // Only cancel if the animation is actually running or has been started and
        // is about
        // to run
    ...
    /**
     * Internal function called to process an animation frame on an animation that is
     * currently sleeping through its <code>startDelay</code> phase. The return value
     * indicates whether it should be woken up and put on the active animations
     * queue.
     * 
     * @param currentTime
     *            The current animation time, used to calculate whether the animation
     *            has exceeded its <code>startDelay</code> and should be started.
     * @return True if the animation's <code>startDelay</code> has been exceeded and
     *         the animation should be added to the set of active animations.
     */
    boolean delayedAnimationFrame(long currentTime) {
        if (!mStartedDelay) {
            mStartedDelay = true;
     ...
     * animation. The currentTime parameter is the timing pulse sent by the handler,
     * used to calculate the elapsed duration, and therefore the elapsed fraction, of
     * the animation. The return value indicates whether the animation should be
     * ended (which happens when the elapsed time of the animation exceeds the
     * animation's duration, including the repeatCount).
     * 
     * @param currentTime
     *            The current time, as tracked by the static timing handler
     * @return true if the animation's duration, including any repetitions due to
     *         <code>repeatCount</code> has been exceeded and the animation should be
     *         ended.
     */
     ...
     * animation frame. This function turns the elapsed fraction into an interpolated
     * fraction and then into an animated value (from the evaluator. The function is
     * called mostly during animation updates, but it is also called when the
     * <code>end()</code> function is called, to set the final value on the property.
     * 
     * <p>
